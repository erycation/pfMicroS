...
@Component({
  selector: 'app-add-group-leaderboard',
  templateUrl: './add-group-leaderboard.component.html',
  styleUrls: ['./add-group-leaderboard.component.css']
})
export class AddGroupLeaderboardComponent extends ModalResetParams implements OnInit {
  ...
  async ngOnInit() {
    this.SiteDto = this.userSites.find(userSite => userSite.siteID === 0);
    this.leaderBoardPromotionDto.siteId = this.SiteDto.siteID;
    this.leaderBoardPromotionDto.startRank = 1;
    this.leaderBoardPromotionDto.endRank = 1;
  }
  ...
  async saveLeaderBoard() {
    ...
    if (!SharedFunction.checkUndefinedObjectValue(this.leaderBoardPromotionDto.siteId)) {
      this.dialogService.openAlertModal("Alert", "Invalid Unit", () => { });
      return;
    }
